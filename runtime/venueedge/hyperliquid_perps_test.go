...
func TestHyperliquidPerpsVenue(t *testing.T) {
	...
	makeAdapter := func(t *testing.T, handler http.Handler) *HyperliquidPerpsVenue {
		t.Helper()
	...
	t.Run("surfaces non-success statuses venue errors and malformed payloads", func(t *testing.T) {
		...
		malformedAdapter := makeAdapter(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			var payload map[string]any
			if decodeErr := json.NewDecoder(r.Body).Decode(&payload); decodeErr != nil {
				t.Errorf("decode request body: %v", decodeErr)
				w.WriteHeader(http.StatusInternalServerError)
			...
			case hyperliquidInfoTypeTrades:
				fmt.Fprint(w, `[{"coin":"ETH","px":"62001.5","sz":"0.125","time":1710000005000,"tid":101}]`)
			default:
				http.NotFound(w, r)
			}
		}))
