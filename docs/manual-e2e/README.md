...
3. Prepare backend tables before starting or restarting backend processes:
   ...
   - `cd ../..`
4. From the repo root, ensure backend is running and fresh:
   - `pm2 status`
...
5. If backend startup reports `bind: address already in use`, stop the stray process already listening on `127.0.0.1:4501` before retrying. A common cause is an older direct `go run ./cmd/signal-foundry start ...` process launched from the app root.
