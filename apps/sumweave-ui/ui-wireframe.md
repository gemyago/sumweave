...
- Body order:
  ...
  - second row: full-width paged **Transactions** list scoped to the selected reporting range. Selecting either bar in a cash-flow bucket temporarily filters this list to the bucket's time window; selecting the same bucket again or using **Clear chart filter** restores the reporting range. The chart's accessible name and linked exact description preserve discoverability without persistent instructional copy. Below the chart at mobile widths, a native time-window selector provides reliably sized targets for the same bucket selection and reset behavior. The selected bucket is visually emphasized, its exact range is shown above the list, and **View all transactions** carries the effective inclusive range into the ledger.
