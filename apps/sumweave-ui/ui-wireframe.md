# Sumweave UI — wireframe (composition, states, behavior)
...
- **Nav — left:** Brand label **Sumweave** → `/finance`.
- **Nav — center:** **Finance** / **Chat** / **Providers** / **Admin** share the same centered max-width column as `main-inner` on non-chat routes.
...
- **`<main>`:** Centered content wrapper `main-inner` (~800–900px max width) with `Router` inside; Finance and admin routes can widen it.
...
- Default **Auto** (follows `prefers-color-scheme`); choice stored in `localStorage` (`sumweave-ui-theme`).
...
- `/chat`, `/finance*`, `/admin*`, and `/providers`: protected with `svelte-spa-router`’s `wrap` + `conditions`.
...
- **Document titles:** Route pages own their title by rendering `DocumentTitle`, which supplies the `<svelte:head>` entry. Use current location first, then parent section when useful, then `Sumweave` (for example, `Accounts · Finance · Sumweave`). Loaded account detail names replace the generic detail label; fallback stays generic while loading or unavailable.
...
| `/finance` | Canonical Bootstrap finance dashboard. Protected. Uses the shared Finance shell, keeps current-FX-valuation period-net summaries in the first viewport, exposes previous/current/next, last-six-months, last-twelve-months, and custom reporting-period controls, charts settled cash flow through its separate series read, caps account/category/recent-transaction sections, and keeps missing/stale FX plus sync/import follow-up visible. |
...
| `/finance/rules` | Finance classification rules manager. Protected. |
...
- **Main column:** Header (“Chat” page title), then **chat column** in order:
  ...
  - **Turn activity** → optional **model/profile banners** (loading, error, or empty) → **composer** (textarea, then **execution profile** `<select>` + **model** `<select>` on the left and **Send** on the right) **pinned to the bottom** of the viewport.
...
- No profile is auto-selected; the operator explicitly selects a retained generic profile.
...
- Finance routes always render inside one shared Bootstrap Finance shell.
...
- After selection, the active tenant is reused across `#/finance`, `#/finance/accounts`, `#/finance/accounts/new`, `#/finance/accounts/:accountId`, `#/finance/transactions`, `#/finance/transactions/new`, `#/finance/transactions/:transactionId`, `#/finance/categories`, `#/finance/rules`, `#/finance/connections`, `#/finance/imports`, and `#/finance/jobs/:jobId` until changed.
...
- Header: compact **Finance dashboard** heading, short current-FX context, selected period/mode, reporting controls, and direct **Add transaction** action. Accounts and transactions remain available through the Finance rail.
- This is the default authenticated landing when there is no remembered protected route.
- Top area: compact dashboard header and direct transaction-entry action. The selected reporting range persists as date-only inclusive `startDate`/`endDate` hash query parameters, so reloads and direct dashboard URLs retain it.
- Controls area: date-only reporting-period summary, direct **Previous month**, **Current month**, **Next month**, **Last 6 months**, and **Last 12 months** controls, and a compact custom date-range disclosure with an **Apply** action. Tenant control is not repeated here. Period controls disable while a dashboard request is active. Direct month actions calculate local calendar-month start and the next local month start; last-six and last-twelve-month actions include the current month plus its preceding five or eleven local calendar months. All actions send explicit half-open `[startDate, endDate)` bounds. Inclusive end labels show the previous local calendar day for an exclusive end at local midnight; otherwise they show the exclusive end instant's local calendar date. A tenant change reloads the active client range; responses from an old tenant cannot replace the new tenant's dashboard or range. **Current month** restores the browser's current local month. Custom ranges remain separate.
...
- Body order:
  - first row: full-width primary **Cash flow over time** card with a compact header: title at left and settled Net state plus settled transaction count at right. When FX makes the settled totals incomplete, the count explicitly says **valued settled transactions**. Its KPI strip contains only Net, Income, Expense, and Pending net values; it uses a compact `2 x 2` grid through large widths, then four columns at the extra-large desktop breakpoint. The `p-2` mobile tiles and `fs-6` single-line values contain at least `100000.00 USD` without clipping or document overflow. Grouped settled-income and settled-expense bars come from the independent cash-flow series read. The chart keeps an accessible name and linked exact textual description, unambiguous included month/year labels for monthly groupings, responsive tick-label reduction, and exact bucket tooltip values.
  - The cash-flow card places incomplete settled-total warnings with the KPIs and links to FX diagnostics. It does not disclose native settled totals or individual FX-rate metadata.
  - second row: full-width paged **Transactions** list scoped to the selected reporting range. Selecting either bar in a cash-flow bucket temporarily filters this list to the bucket's time window; selecting the same bucket again or using **Clear chart filter** restores the reporting range. The chart's accessible name and linked exact description preserve discoverability without persistent instructional copy. Below the chart, a native time-window selector provides reliably sized targets for the same bucket selection and reset behavior at every viewport. The selected bucket is visually emphasized only after its rows load successfully, its exact range is shown above the list, and **View all transactions** carries exact selected-bucket boundaries into the ledger.
  - third row: capped top-category section and capped account snapshot section with the booked account-balance total, plus links into the dedicated browse/detail routes, followed by visually secondary compact needs-attention cards for pending activity, missing FX, sync failures, and import follow-up.
- When dashboard data is available, the first useful viewport should read in this order: compact header and period context, cash-flow heading, period-net-first KPI strip, one primary visual summary, then capped activity and attention states. The period net is settled income minus expenses for the selected period; account balances remain secondary snapshot information.
...
- The chart read is independent from dashboard summary loading. Its card alone shows loading, no-settled-activity, partial-FX diagnostics, or a recoverable retry error. An incomplete all-zero response shows its partial-FX warning and missing-pair diagnostics before the no-settled-activity message, and still provides the loaded bucket chart description. A late series response cannot replace the selected tenant/range/grouping's current chart. Current, previous, and next month use daily buckets; last-six, last-twelve, and custom ranges longer than 31 local calendar dates use monthly buckets.
...
- A stale rate (the backend threshold) produces a prominent warning and refresh link. Missing coverage names distinct pairs and affected values, never occurrence counts as “rates” or “gaps”; display totals stay partial/unavailable rather than substituting native minor units. The dashboard routes operators to FX diagnostics rather than repeating rate metadata or native-total disclosures.
...
- Account detail: loads the account directly through get-account, including hidden historical accounts. The displayed account name is the page title at the summary's left; compact kind, currency, provider, and hidden-state labels align opposite it. Its document title is `{account name} · Accounts · Sumweave`; a compact icon-only **Edit account name** action sits beside the title; opening it replaces that name area with a labeled name input and Save/Cancel controls. Hide confirmation and restore remain separate summary actions. Hiding is never described as deletion: copy says it removes the account from current dashboard reporting and new transactions while preserving history and provider sync. A Hidden historical source badge stays visible. The full-width summary is followed by a full-width recent-transactions card. Recent activity uses the shared responsive transaction list and fixed 10-row offset pages with reusable button-only pager controls; it resets to page one when the account or active tenant changes. During a pager request, the list and pager region remain mounted; on success the shared pager preserves its viewport top by compensating the exact post-commit delta, then focuses that pager region with `preventScroll: true`. Its absolute `scrollY` may change when a final short page shrinks content above it; failures do neither and preserve the current page. The summary includes a compact native **Provider source data** disclosure that is collapsed by default and expands to the summary width. Opening it lazily loads current snapshot metadata only; `account` and `account_balance` kinds remain distinct rows even when they share a provider object. Each row identifies its kind, provider object, and capture time, with an explicit **Reveal source data** action that lazily loads its document. Copy explains that each document is the latest schema-derived provider snapshot rather than a raw HTTP response; no history affordance is present. Loading, empty, metadata-error, and detail-error states stay inside the disclosure and errors provide local retry actions.
...
- Transactions browse route: Bootstrap filter card, tenant/account/type/date-range/sort filters, route-level action links, and visible summary chips. Status and source are not browse filters. Active filters persist in the hash URL; an absent date range remains unfiltered.
...
- After a successful inline category assignment or change, the row offers a separate optional rule form. It is prefilled from the saved transaction description and category, lets the operator choose `contains` or `exact` and edit the condition/category, and requires an explicit save. Canceling or a rule-save failure leaves the transaction category saved and the form recoverable. Clearing a category does not show the offer.
- Every successful inline category assignment creates a distinct optional-rule offer. A replacement offer resets the form's match type, condition, and category to its latest saved defaults even if they equal the prior offer; ordinary rerenders of the same offer retain the operator's edits.
- The compact offer presents text actions only until opened: **Create rule from this transaction** and **Dismiss**. Its expanded form includes optional existing-tag checkboxes and captures the source transaction's latest saved tags only when opened.
...
- Transfer and source-data refinement: candidate lookup returns other-account rows, so normal picker rows do not show a red eligibility-error affordance; client-side validation still disables a stale invalid candidate before confirmation. The collapsed **Provider source data** disclosure presents current schema-derived snapshots as distinct kind/provider-object rows, never a history timeline.
- Transaction source-data states: existing detail routes use the same collapsed-by-default disclosure as account detail. Expansion lazily loads current `transaction` snapshot metadata; **Reveal source data** explicitly loads the sanitized schema-derived provider transaction document. Loading, empty, metadata-error, and detail-error feedback remains bounded inside the disclosure with local retry actions. Only current snapshots are shown.
...
- Category rows include an accessible icon-only **Remove** action with an in-row confirmation. A successful removal refreshes the active category list and confirms that historical transaction references remain. Pending/error state stays local and recoverable. When the server reports that live classification rules reference the category, the row remains visible and the route explains that those rules must be retargeted or deleted; it identifies returned rule IDs and links to the Rules route filtered to that category. Tag management remains edit/create only.

**Rules (`/finance/rules`)**

- Header: **Classification rules** heading, concise top-to-bottom precedence explanation, and compact **Add rule** action.
- Rules are an ordered, accessible Bootstrap list. Every row shows position, `exact`/`contains`, the condition, and its target category's name (never an opaque category ID).
...
- Each row names its target category and renders any target tags as compact badges; missing catalog entries remain visibly unavailable rather than appearing as opaque assignments.
- Add/edit uses one local form with match type, editable condition, target-category select, optional existing-tag checkbox group, explicit save, and cancel. A successfully loaded empty tag catalog allows a category-only rule. A failed or stale catalog preserves selected IDs, blocks the invalid save, and offers retry or explicit removal without discarding the rest of the draft. Empty, loading, tenant-selection, and recoverable error states remain bounded to the page.
...
- Each connection card shows provider/state plus a stable secondary identifier: a non-empty provider reference, otherwise its created time, and finally its internal connection ID. It also shows last sync outcome, schedule visibility, job deep links, and an inline icon-only **Rename** pencil. Each Rename control's accessible name includes the card's display name and stable secondary identifier. Rename replaces that card's title with a labeled, focused name input and icon-only Save/Cancel controls with accessible names; a successful save updates only that card, announces success politely, and returns focus to that card's Rename control. Cancel also returns focus to Rename. A failure preserves its draft, renders a visible card-local error associated with the name input, and retains focus in the editor for recovery. Rename, save, and cancel use the shared transaction inline-edit action behavior, including 44px minimum touch targets on narrow screens. Cards also provide an in-card delete confirmation that repeats the selected row identifier before removing it immediately after a successful delete. Each **Sync now** action owns its compact live job-status widget: a recently returned dispatch ID may show pending during the pre-materialization `404`, then it polls queued/running lifecycle state, stops terminally, retains a Finance job link, and never overwrites another connection’s status. A native **Synced accounts** disclosure lazily loads safe account name, currency, last-successful-sync, and account links once per card; each card owns loading/error/retry state, and **Sync now** invalidates that card’s cache.
...
- A `404` remains an error for direct or arbitrary job-detail links. Only a job-observed dispatch ID returned in the current SPA flow is remembered as pending: its inline status and opened detail poll until the worker materializes the job. This pending state has no client timeout; bounded diagnostics are deferred follow-up work.
...
- The admin FX widget treats a pre-materialization `404` as pending only for the observed dispatch ID it just received; admin job-list and deep links keep ordinary `404` errors.
- `#/admin/finance/providers` shows sanitized provider readiness plus recent finance-job summaries only; no secrets or provider source documents are shown.
---
