...
- The chart read is independent from dashboard summary loading. Its card alone shows loading, no-settled-activity, partial-FX diagnostics, or a recoverable retry error. A late series response cannot replace the selected tenant/range/grouping's current chart. Current, previous, and next month use daily buckets; last-six, last-twelve, and custom ranges longer than 31 local calendar dates use monthly buckets.
