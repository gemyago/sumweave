...
- Body order:
  - first row: full-width primary **Cash flow over time** card with its period-net-first KPI strip (Net, Income, Expense, Pending net), grouped settled-income and settled-expense bars from the independent cash-flow series read, and a collapsed exact textual alternative. The KPI strip is `2 x 2` on narrow screens and four columns on desktop. The chart keeps an accessible name, unambiguous included month/year labels for monthly groupings, responsive tick-label reduction, and exact bucket tooltip values.
  - The cash-flow card places incomplete settled-total warnings with the KPIs and keeps collapsed **Valuation details** below the chart for native totals and FX coverage. Native currencies remain separate and are never combined into a display-currency total.
  ...
  - fourth row: full-width paged **Transactions** list scoped to the selected reporting range, followed by visually secondary compact needs-attention cards for pending activity, missing FX, sync failures, and import follow-up. **View all transactions** carries that same inclusive range into the ledger.
...
- The chart read is independent from dashboard summary loading. Its card alone shows loading, no-settled-activity, partial-FX diagnostics, or a recoverable retry error. An incomplete all-zero response shows its partial-FX warning and missing-pair diagnostics before the no-settled-activity message, and still provides the loaded bucket textual alternative. A late series response cannot replace the selected tenant/range/grouping's current chart. Current, previous, and next month use daily buckets; last-six, last-twelve, and custom ranges longer than 31 local calendar dates use monthly buckets.
