...
- Top area: compact dashboard header plus direct links into accounts and transactions. The selected reporting range persists as date-only inclusive `startDate`/`endDate` hash query parameters, so reloads and direct dashboard URLs retain it.
...
- Body order:
  - first row: period-net story plus compact income/expense/pending-net summaries
  - second row: full-width primary **Cash flow over time** card with grouped settled-income and settled-expense bars from the independent cash-flow series read; it keeps an accessible name, unambiguous included month/year labels for monthly groupings, responsive tick-label reduction, exact bucket tooltip values, and a collapsed textual alternative that lists every bucket range and its exact values
  - third row: capped top-category section and capped account snapshot section with the booked account-balance total, plus links into the dedicated browse/detail routes
