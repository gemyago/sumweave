...
- Controls area: date-only reporting-period summary, direct **Previous month**, **Current month**, and **Next month** controls, and a compact custom date-range disclosure with an **Apply** action. Tenant control is not repeated here. Period controls disable while a dashboard request is active. The client calculates local calendar-month start and end instants for every direct month action and sends those explicit `startDate` and `endDate` values. A tenant change reloads the active client range; responses from an old tenant cannot replace the new tenant's dashboard or range. **Current month** restores the browser's current local month. Custom ranges remain separate.
