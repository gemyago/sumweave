...
### Primary
- **Primary Dark** (`#201d1d`): Primary background, button fills, link text on light theme. A warm near-black with subtle reddish-brown warmth -- rgb(32, 29, 29). CSS token: `--color-primary-dark`.
- **Primary Light** (`#fdfcfc`): Primary text on dark surfaces, button text on dark theme. A barely-warm off-white. CSS token: `--color-primary-light`.
...
**Implementation (CSS — `src/styles/design-system.css`):** Document color grids with `ds-color-grid`, `ds-color-group-label`, `ds-swatch`, `ds-swatch__info`, `ds-swatch__name`, `ds-swatch__hex`, `ds-swatch__role`. Swatch faces: `ds-swatch__block--primary-dark`, `--primary-light`, `--mid-gray`, `--dark-surface`, `--light-surface`, `--input-bg`, `--border-gray`, `--text-muted`, `--accent`, `--accent-hover`, `--accent-active`, `--danger`, `--success`, `--warning`, `--text-secondary`. **Tokens** (palette variables) live in **`src/app.css`** — design system **layer 1**; see §10.
...
**Implementation (CSS):** Scale samples — `ds-type-h1`, `ds-type-h2`, `ds-type-body-medium`, `ds-type-body`, `ds-type-tight`, `ds-type-caption`, `ds-type-code`, `ds-type-meta`; wrap each sample + meta in `ds-type-sample`.
...
**Primary (Dark Fill)**
- Background: `#201d1d` (Primary Dark)
- Text: `#fdfcfc` (Primary Light)
- Padding: 4px 20px
- Radius: 4px
...
**Implementation (CSS):** Buttons — `ds-btn`, `ds-btn--primary`, `ds-btn--secondary`, `ds-btn--accent`, `ds-btn--danger`, `ds-btn--success`, `ds-btn--warning`; rows `ds-btn-row`, `ds-btn-item`, `ds-btn-item__label`. Forms — `ds-form-group`, `ds-label`, `ds-input`, `ds-input--focus`, `ds-input--error`, `ds-form-hint`. Cards — `ds-card-grid`, `ds-card`, `ds-card--outline`, `ds-card__title`, `ds-card__body`, `ds-card__badge`, `ds-card__badge--accent|success|warning`. The app also uses `button.primary` / `button.secondary` / `button.danger` in `app.css` for shell controls.
...
**Implementation (CSS):** Hero — `ds-hero`, `ds-hero__title`, `ds-hero__lead`, `ds-hero__actions`. Sections — `ds-section`, `ds-section-label`, `ds-section-title`, `ds-divider`. Optional reference-style nav (not the main app chrome) — `ds-nav`, `ds-nav__start`, `ds-nav__center`, `ds-nav__end`, `ds-nav__brand`, `ds-nav__cta`, `ds-nav__github`.
...
**Shadow Philosophy**: Depth is intentionally **flat**: no box-shadows in the token set. Hierarchy comes from border treatments and background shifts only — consistent with a terminal-like UI. The three border levels (transparent warm, tab indicator, solid outline) are enough without elevation shadows.

**Implementation (CSS):** Demos — `ds-elevation-grid`, `ds-elevation-card`, `ds-elevation-card--flat`, `--subtle`, `--tab`, `--outline`, `ds-elevation-card__label`, `ds-elevation-card__desc`. Spacing scale — `ds-spacing-row`, `ds-spacing-item`, `ds-spacing-bar` (set width with `style="--ds-space: Npx"`), `ds-spacing-value`. Radius — `ds-radius-row`, `ds-radius-box--4`, `ds-radius-box--6`, `ds-radius-label`, `ds-radius-context`.
...
## 10. Stylesheets and CSS classes

The **CSS design system** is this document reflected in code: **two layers**, always loaded in order from `src/main.ts` — **`app.css` first**, then **`styles/design-system.css`**. Nothing here is a separate “app theme” competing with the system; **`app.css` is layer 1 (foundation)** of the same framework **`design-system.css` extends (layer 2)**.

### 10.1 What “tokens” means here

**Tokens** are **named CSS custom properties** (`--color-primary-dark`, `--space-8`, `--radius-default`, …) and **semantic aliases** mapped per theme (`--bg`, `--text`, `--btn-primary-bg`, …). They store design decisions **once** so light/dark and components stay aligned. Raw hex in components is the exception; prefer `var(--…)` from `app.css`.

### 10.2 Layer 1: `src/app.css` (foundation)

| Responsibility | Contents |
|----------------|----------|
| **Tokens** | Palette, spacing, radii, typography scale on `:root`; theme overrides on `[data-theme]` / default dark. |
| **Semantic aliases** | Per-theme `--bg`, `--text`, `--border`, `--btn-primary-*`, etc. |
| **Global defaults** | `html`, `body`, `#app`, `h1`/`h2`, `a`, `code`. |
| **Shell utilities** | `button.primary`, `button.secondary`, `button.danger`, `.counter` — global classes wired in the SPA today. Same variables as the rest of the system; not a second palette. |

### 10.3 Layer 2: `src/styles/design-system.css` (`ds-*` primitives)
...
### 10.4 External token previews (optional)

Third-party sites may host HTML previews generated from a `DESIGN.md`-style file. Those pages are **not** this repo: they often load **IBM Plex Mono** from Google Fonts, while our stack lists **Berkeley Mono** first in `app.css`. They may also remap `:root` for dark mode independently of our `[data-theme]` blocks. Treat them as **visual references**, not a second source of truth — **DESIGN.md + the two-layer CSS stack (§10)** win for Sumweave UI.

### 10.5 Class index (`design-system.css`)

Sumweave UI ships reusable classes in **`src/styles/design-system.css`** (imported from `src/main.ts` after `app.css`). All use the **`ds-`** prefix. Theme follows `html[data-theme="light"|"dark"]` and **tokens from layer 1** (`app.css`).

| Topic | Classes |
|-------|---------|
| Color swatches | `ds-color-grid`, `ds-color-group-label`, `ds-swatch`, `ds-swatch__block`, `ds-swatch__block--primary-dark`, `…--primary-light`, `…--mid-gray`, `…--dark-surface`, `…--light-surface`, `…--input-bg`, `…--border-gray`, `…--text-muted`, `…--accent`, `…--accent-hover`, `…--accent-active`, `…--danger`, `…--success`, `…--warning`, `…--text-secondary` |
| Typography samples | `ds-type-h1`, `ds-type-h2`, `ds-type-body-medium`, `ds-type-body`, `ds-type-tight`, `ds-type-caption`, `ds-type-code`, `ds-type-meta`, `ds-type-sample` |
| Buttons | `ds-btn`, `ds-btn--primary`, `ds-btn--secondary`, `ds-btn--accent`, `ds-btn--danger`, `ds-btn--success`, `ds-btn--warning`, `ds-btn-row`, `ds-btn-item`, `ds-btn-item__label` |
| Forms | `ds-form-group`, `ds-label`, `ds-input`, `ds-input--focus`, `ds-input--error`, `ds-form-hint` |
| Cards | `ds-card-grid`, `ds-card`, `ds-card--outline`, `ds-card__title`, `ds-card__body`, `ds-card__badge`, `ds-card__badge--accent`, `--success`, `--warning` |
| Hero & sections | `ds-hero`, `ds-hero__title`, `ds-hero__lead`, `ds-hero__actions`, `ds-section`, `ds-section-label`, `ds-section-title`, `ds-divider` |
| Optional nav pattern | `ds-nav`, `ds-nav__start`, `ds-nav__center`, `ds-nav__end`, `ds-nav__brand`, `ds-nav__cta`, `ds-nav__github` |
