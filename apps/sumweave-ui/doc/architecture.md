# Sumweave UI architecture (brief)
...
Browser SPA for Sumweave under `apps/sumweave-ui`: static **`dist/`** from Vite, no SSR in this iteration. Final goal is to have the UI embeddable in a backend app and deployed as a single unit.
...
- **Document titles** — route pages render `DocumentTitle` (which owns `<svelte:head>`)
  with `documentTitle()`;
  current page comes first, then its section, then `Sumweave`.
...
- **`apps/sumweave-ui`** is its own npm package: **`package-lock.json`**, **`npm ci`** for installs.
- **`apps/sumweave-ui/Makefile`:** `lint` → `npm run lint`, `test` → `npm run test:run`, `build-embed-dist` → `vite build --outDir … --emptyOutDir` plus `index.html` validation for backend embedding.
- **Root `Makefile`** runs **`$(MAKE) -C apps/sumweave-ui lint`** and **`test`**. JS coverage is not merged into Go’s merged HTML report; UI failures still fail the root **`make test`**.
- **Backend build:** `apps/sumweave/make dist/bin` calls `apps/sumweave-ui/make build-embed-dist` so the Go binary embeds the current SPA output for same-origin `/` + `/api/v1/*` delivery.
...
- **Isolated lockfile in `apps/sumweave-ui`:** matches monorepo layout; root stays Go-centric.
