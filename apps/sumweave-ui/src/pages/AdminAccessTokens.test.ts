...
describe('Admin access tokens page', () => {
  ...
  it('refreshes server metadata after rotation while retaining the one-time secret', async () => {
    const user = userEvent.setup()
    const original = tokenFixture()
    const revokedOriginal = { ...original, status: 'revoked' as const, revokedAt: faker.date.recent().toISOString() }
    const replacement = tokenFixture()
    const replacementSecret = faker.string.alphanumeric(48)
    mocks.list.mockResolvedValueOnce([original]).mockResolvedValueOnce([replacement, revokedOriginal])
    mocks.rotate.mockResolvedValue({ token: replacement, apiToken: replacementSecret })
    render(AdminAccessTokens)
    ...
    expect(mocks.rotate).toHaveBeenCalledWith(original.id, { expiresAt: null })
    expect(await screen.findByText(new RegExp(`${original.hint} · read-only · revoked`))).toBeInTheDocument()
    expect(screen.getByText(new RegExp(`${replacement.hint} · read-only · active`))).toBeInTheDocument()
    expect(screen.getByLabelText('Issued access token')).toHaveTextContent(replacementSecret)
    expect(mocks.list).toHaveBeenCalledTimes(2)
  })

  it('confirms revoke and refreshes revoked metadata', async () => {
    const user = userEvent.setup()
    ...
    expect(screen.getByRole('heading', { name: 'Confirm revoke' })).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Confirm revoke' }))
    expect(mocks.revoke).toHaveBeenCalledWith(token.id)
    expect(mocks.list).toHaveBeenCalledTimes(2)
  })
