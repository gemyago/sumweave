...
func TestNewAppErrorHandler(t *testing.T) {
	...
	t.Run("UnauthorizedError sets 401 with empty body", func(t *testing.T) {
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		err := app.NewErrUnauthorized("unauthorized")
		handler(w, req, err)
		assert.Equal(t, http.StatusUnauthorized, w.Code)
		assert.Empty(t, w.Body.String())
	})

	t.Run("auth ErrInvalidCredentials sets 401 with empty body", func(t *testing.T) {
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		handler(w, req, auth.ErrInvalidCredentials)
		assert.Equal(t, http.StatusUnauthorized, w.Code)
		assert.Empty(t, w.Body.String())
	})
