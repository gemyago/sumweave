...
func TestRecover(t *testing.T) {
	...
	t.Run("should call next", func(t *testing.T) {
		nextCalled := true
		// Status must allow a response body (not 204/304/1xx); json.Encode fails otherwise.
		wantNextStatus := 200 + rand.Intn(99)
		if wantNextStatus == http.StatusNoContent {
			wantNextStatus = http.StatusOK
		}
