...
func TestMuxRouterAdapter(t *testing.T) {
	...
	t.Run("Handle registers without router middleware", func(t *testing.T) {
		calls := []string{}
		adapter := NewHTTPRouter(HTTPRouterDeps{
			Middleware: func(h http.Handler) http.Handler {
				calls = append(calls, "middleware")
		...
		adapter.Handle(http.MethodGet+" /plain", http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) {
			calls = append(calls, "handler")
		}))
	...
	t.Run("Handle supports nested routers with middleware", func(t *testing.T) {
		calls := []string{}
		parent := NewHTTPRouter(HTTPRouterDeps{
			Middleware: func(h http.Handler) http.Handler {
				calls = append(calls, "middleware")
				return h
			},
		...
		childMux.Handle(http.MethodGet+" /child", http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) {
			calls = append(calls, "child handler")
		}))

		parent.Handle("/parent/", http.StripPrefix("/parent", childMux))
		req := httptest.NewRequest(http.MethodGet, "/parent/child", http.NoBody)
		parent.ServeHTTP(httptest.NewRecorder(), req)
		assert.Equal(t, []string{"middleware", "child handler"}, calls)
	})
}
