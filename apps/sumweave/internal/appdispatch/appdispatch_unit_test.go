...
func TestAppDispatchUnits(t *testing.T) {
	...
	t.Run("normalizes config and detects drivers", func(t *testing.T) {
		...
		assert.Equal(t, "sumweave-app-dispatch", normalized.ConsumerName)
	...
	t.Run("builds sqlite table generators and migration queries for sqlite and postgres", func(t *testing.T) {
		cfg := Config{TablePrefix: "sumweave_data_"}
		...
		postgresCfg := Config{TablePrefix: "sumweave_data_"}
		assert.Contains(
			t,
	...
	t.Run("covers explicit migrator behavior on sqlite and postgres", func(t *testing.T) {
		...
		t.Run("sqlite migration creates schema and stays idempotent", func(t *testing.T) {
			cfg := Config{DatabaseDSN: makeSQLiteMemoryDSN("sqlite-migrator"), TablePrefix: "sumweave_data_"}
		...
		t.Run("sqlite migration surfaces write failures", func(t *testing.T) {
			...
			migrator, err := NewMigrator(Config{TablePrefix: "sumweave_data_"}, readOnlyDB)
			require.NoError(t, err)
		...
		t.Run("postgres migration executes generated schema queries", func(t *testing.T) {
			...
			migrator, err := NewMigrator(
				Config{
					DatabaseDSN: "postgres://sumweave:secret@example.invalid:5432/sumweave?sslmode=disable",
					TablePrefix: "sumweave_data_",
				},
				postgresDB,
			)
			require.NoError(t, err)
