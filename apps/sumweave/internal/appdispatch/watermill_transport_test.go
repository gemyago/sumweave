...
func TestWatermillTransport(t *testing.T) {
	...
	t.Run("uses the repo sqlite-first database shape and supports transactional publish", func(t *testing.T) {
		...
		require.Equal(t, "sumweave_data_app_dispatch_messages", cfg.MessagesTable())
		require.Equal(t, "sumweave_data_app_dispatch_offsets", cfg.OffsetsTable())
		...
		err = db.QueryRowContext(
			t.Context(),
			`SELECT COUNT(*) FROM sumweave_data_app_dispatch_messages`,
		).Scan(&messageCount)
		require.NoError(t, err)
	...
	t.Run("surfaces publish and consume failures without schema side effects", func(t *testing.T) {
		...
		_, err = db.ExecContext(
			t.Context(),
			`INSERT INTO sumweave_data_app_dispatch_messages (uuid, created_at, payload, metadata) VALUES (?, CURRENT_TIMESTAMP, ?, '{}')`,
			"msg-"+fake.UUID().V4(),
			[]byte(`not-json`),
		)
		require.NoError(t, err)
