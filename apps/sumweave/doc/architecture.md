...
- **CLI user subcommands** (`cmd/sumweave`, e.g. **`user add` / `list` / `change-password`**) obtain **`auth.UserStore`** and **`auth.Argon2idHasher`** by calling **`NewEngine`** with the same injected **`dig.Container`** as the command (`internal.WithEngineContainer`), then resolving those types from that container—no extra **`Engine`** getters.
