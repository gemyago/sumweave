...
- **`Engine`** (`engine.go`, package **`sumweave`**) holds the **`dig.Container`** and **`viper.Viper`** produced by **`internal.Setup`**. It is the embeddable entrypoint when you do not run **`cmd/sumweave`** (tests, other binaries, library-style use).
...
- **CLI user subcommands** (`cmd/sumweave`, e.g. **`user add` / `list` / `change-password`**) obtain **`auth.UserStore`** and **`auth.Argon2idHasher`** by calling **`NewEngine`** with the same injected **`dig.Container`** as the command (`internal.WithEngineContainer`), then resolving those types from that container—no extra **`Engine`** getters. **`user add --if-not-exists`** makes deployment bootstrap retries succeed without changing an existing password.
...
- **`sumweave jobs worker`** / **`sumweave jobs enqueue-due`** — dedicated split-environment consumer and one-shot scheduler commands.
