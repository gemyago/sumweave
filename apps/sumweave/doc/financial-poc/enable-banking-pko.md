...
```bash
go run ./cmd/sumweave finance-poc enable-banking start-auth \
  --country PL \
...
```bash
go run ./cmd/sumweave finance-poc enable-banking finish-session \
  --auth-file ../../data/financial-poc/pko.enable-banking-pending-auth.json \
  --code '<callback code>' \
...
```bash
go run ./cmd/sumweave finance-poc enable-banking connect \
  --country PL \
  --aspsp-name "PKO Bank Polski" \
  --psu-type personal \
  --valid-days 90 \
...
```bash
go run ./cmd/sumweave finance-poc enable-banking accounts \
  --session-file ../../data/financial-poc/pko.enable-banking-session.json \
  --include-details \
  --include-balances \
  --json

go run ./cmd/sumweave finance-poc enable-banking transactions \
  --session-file ../../data/financial-poc/pko.enable-banking-session.json \
