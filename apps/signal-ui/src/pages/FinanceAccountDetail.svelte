<script lang="ts">
  ...
  import { createSignalFinanceApiForAuth, type FinanceAccount, type FinanceTransaction } from '../lib/finance/api'
...
<section class="page" aria-labelledby="finance-account-detail-heading">
  ...
  {#if loading}<p class="muted" role="status">Loading account detail…</p>{:else if financeShell.needsTenantSelection}<section class="panel">{#if !financeShell.embedded}<label><span>Tenant</span><select value={financeShell.selectedTenantId} onchange={(event) => financeShell.selectTenant((event.currentTarget as HTMLSelectElement).value)} aria-label="Tenant"><option value="">Select tenant</option>{#each financeShell.tenants as tenant (tenant.id)}<option value={tenant.id}>{tenant.name}</option>{/each}</select></label>{/if}<p>Select an active tenant to continue on this finance route.</p></section>{:else if !financeShell.selectedTenantId}<section class="panel"><p>Create or join a tenant from <a href="/finance/tenants" use:link>Finance tenants</a> before opening this account detail route.</p></section>{:else if account}<section class="panel"><h2>{account.name}</h2><p class="muted">{account.kind} · {account.currency} · Provider {account.provider || 'manual'}</p><p class="muted">Created {formatFinanceDateTime(account.createdAt)} · Updated {formatFinanceDateTime(account.updatedAt)}</p></section><section class="panel"><h2>Recent transactions</h2>{#if transactions.length === 0}<p class="muted">No transactions yet.</p>{:else}<div class="stack">{#each transactions as item (item.id)}<article class="card"><strong>{item.description || item.kind}</strong><span>{item.status}</span><span>{item.amountMinor}</span></article>{/each}</div>{/if}</section>{:else}<p class="muted">Account not found for the selected tenant.</p>{/if}
</section>

<style>.page,.stack{display:flex;flex-direction:column;gap:var(--space-16)}.row{display:flex;justify-content:space-between;gap:var(--space-16);align-items:flex-start}.row-links{display:flex;flex-wrap:wrap;gap:var(--space-12)}.panel{display:flex;flex-direction:column;gap:var(--space-12);padding:var(--space-16);border:1px solid var(--border);border-radius:4px;background:var(--bg-elevated,var(--bg))}.panel h2,header h1{margin:0}.card{display:flex;justify-content:space-between;gap:var(--space-12);padding:var(--space-12);border:1px solid var(--border);border-radius:4px}.muted{margin:0;color:var(--text-muted)}.error{color:var(--color-danger-red)}label{display:flex;flex-direction:column;gap:var(--space-8)}</style>
