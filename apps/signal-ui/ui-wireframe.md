...
- **Finance shell responsive shape:** At narrow widths, the Finance shell keeps the full Bootstrap navigation visible by stacking the aside as a full-width section above the utility header and route content. Its compact horizontal-wrapping link rows preserve usable targets, while duplicate rail and utility labels hide; tenant, theme, and sign-out controls remain shell-owned and wrap below it with no menu-toggle state.
...
- When settled income/expense reporting is incomplete, an unmistakable warning appears directly with those totals. It states the count of excluded transactions and links to FX diagnostics; the later needs-attention area remains secondary.
...
- A compact current-FX block names each provider/pair with market effective and last-successful-refresh times. A stale rate (the backend threshold) produces a prominent warning and manual-refresh link. No current rate or missing-FX diagnostics make display totals partial/unavailable rather than substituting native minor units.
...
- Account detail: a full-width account summary card followed by a full-width recent-transactions card, with backlinks to Accounts and Transactions. Recent activity uses the shared responsive transaction list and fixed 10-row offset pages with reusable button-only pager controls; it resets to page one when the account or active tenant changes. During a pager request, the list and pager region remain mounted; on success the shared pager preserves its viewport top by compensating the exact post-commit delta, then focuses that pager region with `preventScroll: true`. Its absolute `scrollY` may change when a final short page shrinks content above it; failures do neither and preserve the current page. The summary includes a compact native collapsed **Provider evidence** disclosure: opening it loads metadata only; each metadata row requires an explicit **Reveal sanitized details** action before sanitized evidence is shown. Copy distinguishes sanitized evidence from raw provider payloads.
...
- Each connection card shows provider/state plus a stable secondary identifier (provider reference, external id, or created timestamp), along with last sync outcome, schedule visibility, job deep links, and an in-card delete confirmation that repeats the selected row identifier before removing it immediately after a successful delete. A native **Synced accounts** disclosure lazily loads safe account name, currency, last-successful-sync, and account links once per card; each card owns loading/error/retry state, and **Sync now** invalidates that card’s cache.
...
- `#/admin/finance/fx` shows current-rate counts, provider readiness, and a latest-only manual FX refresh form that deep-links into admin job detail. The form has provider/base/quote fields only; it has no historical date-range fields.
