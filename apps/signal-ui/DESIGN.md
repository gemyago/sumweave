...
Third-party sites may host HTML previews generated from a `DESIGN.md`-style file. Those pages are **not** this repo: they often load **IBM Plex Mono** from Google Fonts, while our stack lists **Berkeley Mono** first in `app.css`. They may also remap `:root` for dark mode independently of our `[data-theme]` blocks. Treat them as **visual references**, not a second source of truth — **DESIGN.md + the two-layer CSS stack (§10)** win for Signal UI.

### 10.5 Class index (`design-system.css`)

Signal UI ships reusable classes in **`src/styles/design-system.css`** (imported from `src/main.ts` after `app.css`). All use the **`ds-`** prefix. Theme follows `html[data-theme="light"|"dark"]` and **tokens from layer 1** (`app.css`).
