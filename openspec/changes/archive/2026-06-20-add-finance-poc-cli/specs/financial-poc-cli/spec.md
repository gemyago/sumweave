...
The backend application SHALL provide a lightweight financial POC command tree under the existing `signal-foundry` Cobra binary.

#### Scenario: Finance POC commands are discoverable under the existing binary
- **WHEN** a user runs the CLI command tree
- **THEN** `signal-foundry finance-poc` MUST expose `enable-banking` and `monobank` subcommands from `apps/signal-foundry/cmd/signal-foundry`
...
- **WHEN** a user runs `signal-foundry finance-poc enable-banking aspsps --country PL --json`
...
- **WHEN** a user runs `signal-foundry finance-poc enable-banking start-auth --country PL --aspsp-name ... --psu-type ... --valid-days ... --redirect-url ... --auth-file ... --json`
...
- **WHEN** a user runs `signal-foundry finance-poc enable-banking accounts --session-file ... --include-details --include-balances --json`
...
- **WHEN** a user runs `signal-foundry finance-poc enable-banking transactions --session-file ... --account-id ... --from YYYY-MM-DD --to YYYY-MM-DD --json`
...
- **WHEN** a user runs `signal-foundry finance-poc monobank accounts --json`
...
- **WHEN** a user runs `signal-foundry finance-poc monobank transactions --account 0 --from YYYY-MM-DD --to YYYY-MM-DD --json`
...
- **WHEN** a user reads `apps/signal-foundry/doc/financial-poc/enable-banking-pko.md`
...
- **WHEN** a user reads `apps/signal-foundry/doc/financial-poc/monobank.md`
