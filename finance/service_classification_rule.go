...
func NewClassificationRuleService(args ClassificationRuleServiceArgs) (*ClassificationRuleService, error) {
	...
	return &ClassificationRuleService{
		access: newAccessGuard(args.Access), categories: args.Categories, rules: args.Rules,
...
func (s *ClassificationRuleService) Create(
	...
	condition, category, err := s.validateRule(
		ctx,
		params.TenantID,
		params.MatchType,
		params.Condition,
	...
	rule, err := s.rules.AppendClassificationRule(ctx, domain.ClassificationRule{
		...
		Condition: condition, CategoryID: category.ID, CreatedAt: now, UpdatedAt: now,
	})
	if err != nil {
...
func (s *ClassificationRuleService) Update(ctx context.Context, params UpdateClassificationRuleParams) error {
	...
	err = s.rules.ReplaceClassificationRule(ctx, domain.ClassificationRule{
		...
		MatchType: params.MatchType, Condition: condition, CategoryID: category.ID, UpdatedAt: s.now(),
	})
...
func (s *ClassificationRuleService) validateRule(
	...
	condition string,
	categoryID string,
) (string, domain.Category, error) {
	if matchType != domain.ClassificationMatchTypeExact && matchType != domain.ClassificationMatchTypeContains {
		return "", domain.Category{}, fmt.Errorf("%w: match type", ErrInvalidClassificationRule)
	}
	condition = strings.TrimSpace(condition)
	if condition == "" {
		return "", domain.Category{}, fmt.Errorf("%w: condition", ErrInvalidClassificationRule)
	}
	...
	if err != nil {
		...
		return "", domain.Category{}, fmt.Errorf("get classification rule category: %w", err)
	}
	if category.TenantID != strings.TrimSpace(tenantID) || category.HiddenAt != nil {
		return "", domain.Category{}, ErrCategoryNotFound
	}
	return condition, *category, nil
}
